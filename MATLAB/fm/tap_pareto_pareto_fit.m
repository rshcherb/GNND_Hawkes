function [vPar, vPci, vPerr] = tap_pareto_pareto_fit(vData,M_min,OptArgs)
%
%   Fit the tapered Pareto + Pareto distribution to data: Kagan (GJI 2002 p.523)
%
...
%
    ...
    %                                                    [alpha,  Mc_1,       beta,   w]
    if isempty(OptArgs.Start),      OptArgs.Start =      [1.1     10*M_min    1.1     0.5]; end
    if isempty(OptArgs.LowerBound), OptArgs.LowerBound = [0.0     2*M_min     1.0     0.0]; end
    if isempty(OptArgs.UpperBound), OptArgs.UpperBound = [5.0     1e26        5.0     1.0]; end

    pdffun = @(x,alpha,x_c,beta,w) tap_pareto_pareto_pdf(x,M_min,alpha,x_c,beta,w);
